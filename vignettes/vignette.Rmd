...
l <- vector("list", 9)
for (i in 6:14) {
    try(l[[i - 5]] <- cbind.data.frame(leyes(i), legislatura = i))
}
l2 <- l[lengths(l) != 0]
l3 <- do.call("rbind", l2)
l3 <- as.data.frame(l3)
ggplot(l3) + 
  geom_point(aes(legislatura, 
                 as.numeric(numLey), group = tipo, col = tipo, shape = tipo), 
             position = position_jitterdodge(jitter.height = 0,
                                             jitter.width = 0.25)) +
  scale_x_continuous(breaks = 1:14, expand = expansion()) +
